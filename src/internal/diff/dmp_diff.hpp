...
enum Operation : int8_t { EQUAL=0, INSERT=1, DEL=2 };
   ...
   public:
    ...
    Diffs diff_compute(Range text1, Range text2, Time deadline) {
        ...
        if (text2.size() == 0) {
            ...
            diffs.push_back(Diff(DEL, text1));
            return diffs;
        }

        Range longtext = text1.size() > text2.size() ? text1 : text2;
        Range shorttext = text1.size() > text2.size() ? text2 : text1;
        int i = static_cast<int>(longtext.find(shorttext));
        if (i != -1) {
            // Shorter text is inside the longer text (speedup).
            Operation op = (text1.size() > text2.size()) ? DEL : INSERT;
    ...
    int diff_commonPrefix(Range text1, Range text2) {
        ...
        int n = std::min(static_cast<int>(text1.size()),
    ...
    Stats stats() const {
        ...
        for (const auto &i : result) {
            switch (i.operation) {
                ...
                case DEL: ret.deleted += i.text.size(); break;
            }
        }
        return ret;
    }
